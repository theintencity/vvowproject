...
$master  = WebSocket("64.131.109.97",8081);
...
function process($user,$msg){
  $action = unwrap($msg);
  say("< ".$action);
  $parts = split(" ", $action, 3);
  $method = $parts[0];
  $resource = $parts[1];
  $request_body = (count($parts) > 2 ? $parts[2] : null);
//  $request_body = $parts[2];
  if ( $method == 'POST' && substr($resource, 0, 7) == '/login/') {
//     $user->username = substr($resource, 7);
     $user->param = substr($resource, 7);
//     send($user->socket, '{"id":"' . $user->id . $param["username"] . $param["firstname"] . $param["lastname"] . $param["password"] .
//           $param["email"] . '"}');
         send($user->socket, "userid:" . $user->id . "  " . $user->param );

  }
 // if ($method == 'GET' && $resource == '/login') {
//  if ($method == 'GET' && substr($resource,0,6) == '/login') {
//   $user->username=substr($resource,6);
//	// $result = "TBD";
//	$result = "id:" . $user->id . "," . "username:" .
//$user->username . ";" ;
     // iterate through $users, for each $user create a
     // string like {"id":$user->id, "username":$user->username}
     // concatenate/join all such strings to $result, and return the result
     // as [{...}, {...}, ....]
//     send($user->socket, $result);
//  }
  return;
